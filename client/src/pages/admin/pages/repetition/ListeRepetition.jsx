...
function ListeRepetition() {
    ...
    useEffect(() => {
        const fetchRepetitions = async () => {
            try {
                ...
                const repetitionsWithIds = response.data.model.map((repetition, index) => ({
                    ...repetition,
                    id: index + 1,
                    _id: repetition._id,
                ...
                const filteredRepetitions = repetitionsWithIds.filter((repetition) =>
                    saisonCourante &&
                    saisonCourante.some((cons) => cons._id === repetition._id.toString())
                );
    ...
    const filterRepetitions = () => {
        const filtered = repetitions.filter((repetition) =>
            
            repetition.lieu.toLowerCase().includes(searchValue.toLowerCase()) ||
        repetition.concert.titre.toLowerCase().includes(searchValue.toLowerCase())
        );
    ...
    const handleEditDialogClose = () => {
        const formattedData = {
            ...
            HeureDeb: new Date(`${editedData.DateRep}T${editedData.HeureDeb}`).toISOString(),
            HeureFin:new Date(`${editedData.DateRep}T${editedData.HeureFin}`).toISOString(),
        };
        axios.patch(`http://localhost:8000/api/repetition/update/${selectedRow._id}`, formattedData)
            .then(response => {
                console.log('Data updated successfully:', response.data);
                setOpenEditDialog(false);
                // Rafraîchir les données après la mise à jour
                setRepetitions(prevRepetitions => prevRepetitions.map(rep => rep._id === selectedRow._id ? { ...rep, ...formattedData } : rep));
            })
            .catch(error => {
    ...
    const handleConfirmDelete = () => {
        axios.delete(`http://localhost:8000/api/repetition/annuler/${selectedRow._id}`)
            .then(response => {
                console.log('Data deleted successfully:', response.data);
                setRepetitions(prevRepetitions => prevRepetitions.filter(rep => rep._id !== selectedRow._id));
    ...
    const loadDetailRepetition = (row) => {
        axios.get(`http://localhost:8000/api/repetition/getRepetition/${row._id}`)
            .then(response => {
                setDetailRepetition(response.data);
            })
            .catch(error => {
                console.error('Erreur lors du chargement des détails de répétition :', error);
            });
    };
    ...
    const columns = [
        ...
        { field: 'concert', headerName: 'Nom du concert', width: 170, valueGetter: (params) => params.row.concert.titre, },
    ...
    return (
        <div>
            <h4 className="repetition-title">
                <span className="text-muted fw-light">REPETITION /</span> Liste des répétitions
            </h4>
            <Card className="Card-white-rep">
                <CardContent>
                    <div className="button-container-rep">
                        <NavLink to={`/dashboard/admin/addRepetition`}>
                            <Button variant="contained" color="primary">Ajouter une répétition</Button>
                        </NavLink>
                        <TextField
                    ...
                    <div className="tableRep" style={{ height: 300, width: '100%' }}>
                        <DataGrid rows={searchValue ? filteredRepetitions : repetitions} columns={columns} pageSize={3} />
                    </div>
                </CardContent>
            </Card>
            <Dialog open={openEditDialog} onClose={handleDialogClose}>
                ...
                <DialogContent>
                    ...
                    >
                        {allConcerts.map((concert) => (
                            <MenuItem key={concert._id} value={concert._id}>{concert.titre}</MenuItem>
                        ))}
                    </Select>
                    <TextField
                        label="Lieu"
                ...
                <DialogActions>
                    ...
                    <Button variant="contained" onClick={handleEditDialogClose} color="primary">Modifier</Button>
                </DialogActions>
            </Dialog>
            <Dialog open={openConfirmDialog} onClose={() => setOpenConfirmDialog(false)}>
                <DialogContent>
                    <Typography className="confirmDelete">Êtes-vous sûr de vouloir supprimer cette répétition ?</Typography>
                </DialogContent>
                <DialogActions>
                    <Button onClick={() => setOpenConfirmDialog(false)}>Annuler</Button>
                    <Button variant="contained" onClick={handleConfirmDelete}>Supprimer</Button>
                </DialogActions>
            </Dialog>
            <Dialog open={detailRepetition !== null} onClose={() => setDetailRepetition(null)}>
                <DialogTitle>Détails de la Répétition</DialogTitle>
                <DialogContent className='detailRep'>
                    {detailRepetition && (
                        <div>
                            <Typography variant="subtitle1">Nom du Concert: {detailRepetition.concert.titre}</Typography>
                            <Typography variant="subtitle1">Lieu: {detailRepetition.lieu}</Typography>
                            <Typography variant="subtitle1">Date: {new Date(detailRepetition.DateRep).toLocaleDateString()}</Typography>
                            <Typography variant="subtitle1">Heure de Début: {decrementHour(new Date(detailRepetition.HeureDeb))}</Typography>
                            <Typography variant="subtitle1">Heure de Fin: {decrementHour(new Date(detailRepetition.HeureFin))}</Typography>
                            <Typography variant="subtitle1">Les membres sopranos: <ul>
                        {detailRepetition.membres.soprano.map((membre) => (
                            <li key={membre._id}>{membre.nom} {membre.prenom}</li>
                        ))}
                    </ul></Typography>
                    <Typography variant="subtitle1">Les membres ténors: <ul>
                        {detailRepetition.membres.ténor.map((membre) => (
                            <li key={membre._id}>{membre.nom} {membre.prenom}</li>
                        ))}
                    </ul></Typography>
                    <Typography variant="subtitle1">Les membres basses: <ul>
                        {detailRepetition.membres.basse.map((membre) => (
                            <li key={membre._id}>{membre.nom} {membre.prenom}</li>
                        ))}
                    </ul></Typography>
                    <Typography variant="subtitle1">Les membres altos: <ul>
