...
const nominateMemberById = async (req, res) => {
  ...
  try {
    ...
    if (!absencesResponse || absencesResponse.total_absences >= seuilNominations) {
      return res.status(400).json({ error: "Member does not meet nomination criteria" });
    }
    ...
    const isAlreadyNominated = saisonCourante.nominatedMembers.some(nominee => nominee.memberId.toString() === memberId);
    ...
    res.json({ message: "Member successfully nominated", nominatedMember: member });
  } catch (error) {
    console.error(error);
    res.status(500).json({ error: "Internal Server Error" });
  }
...
const eliminateMemberById = async (req, res) => {
  ...
  try {
    ...
    const isAlreadyEliminated = saisonCourante.eliminatedMembers.some(eliminee => eliminee.memberId.toString() === memberId);
    ...
    res.json({ message: "Member successfully eliminated", eliminatedMember: member });
  } catch (error) {
    console.error(error);
    res.status(500).json({ error: "Internal Server Error" });
  }
...
const listNominatedMembers = async (req, res) => {
  try {
    const saisonCourante = await Saison.findOne({ saisonCourante: true }).populate('nominatedMembers.memberId', 'nom prenom');

    if (!saisonCourante) {
      return res.status(404).json({ error: "Current season not found" });
    }

    if (!saisonCourante.nominatedMembers || saisonCourante.nominatedMembers.length === 0) {
...
const listEliminatedMembers = async (req, res) => {
  try {
    const saisonCourante = await Saison.findOne({ saisonCourante: true }).populate('eliminatedMembers.memberId', 'nom prenom');

    if (!saisonCourante) {
      return res.status(404).json({ error: "Current season not found" });
    }

    if (!saisonCourante.eliminatedMembers || saisonCourante.eliminatedMembers.length === 0) {
...
const consulterProfil = async (req, res) => {
  try {
    ...
    const listeElimines = saison.eliminatedMembers.map(elimine => elimine.memberId.toString());
    const listeNomines = saison.nominatedMembers.map(nomine => nomine.memberId.toString());
...
const fetchAllAbsences = async () => {
  try {
    ...
    const concerts = await Concert.find({ "_id": { $in: saisonCourante.concerts } });
    const repetitions = await Repetition.find({ "_id": { $in: saisonCourante.repetitions } });
    ...
    for (const concert of concerts) {
      if (concert.listeMembres && Array.isArray(concert.listeMembres)) {
        concert.listeMembres.forEach(member => {
          if (member && member.membre && member.presence === false && absencesByMember[member.membre.toString()]) {
    ...
    console.log("Absences by member calculated: ", Object.keys(absencesByMember).length);
...
module.exports = {
  ...
  fetchAbsences,fetchHistoriqueStatus/*,fetchNominatedMembers,fetchEliminatedMembers*/,eliminateChoristeForReason,nominateMemberById,eliminateMemberById,listNominatedMembers,listEliminatedMembers,consulterProfil,fetchAllAbsences,
